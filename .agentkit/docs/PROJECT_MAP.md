...
- Key user flows:
  ...
  - В applicant wizard BIN заявителя может запускать backend-mediated lookup: runtime запрашивает ГБД ЮЛ по `BIN`, затем Kompra по `IIN` руководителя, автозаполняет sourced applicant-поля; ФИО руководителя из Kompra дублируется в RU/KZ поля, а должность руководителя больше не подтягивается из ГБД ЮЛ и остается ручным полем.
...
- Routing approach:
  ...
  - Applicant wizard Ордер 3 в `frontend/index.html` теперь выровнен по `PDF_ORDERS_DETAILED.md`: буквальные названия applicant/OPS/product/document/note полей приведены к PDF-формулировкам, возвращены conditional-поля адресов, добавлен отдельный `Вид сертификации` для `Продукция серийного производства / Партия продукции / Продукция единичного изделия`, а приложение переведено с JSON-helper на табличные строки.
  - Первый applicant step объединяет реквизиты и адрес заявителя в одну секцию; пустые обязательные поля в editable applicant режиме всегда подсвечиваются мягким красным, sourced lookup-поля визуально серые/read-only, а sidebar step labels выровнены по левому краю для длинных названий.
  - Applicant document package в wizard Ордер 3 теперь file-backed и PDF-complete: после создания/обновления draft frontend загружает выбранные applicant-вложения через `POST /files/slots/upload`, сохраняет MinIO metadata в `payload.file_slots`, показывает статусы по слотам и поддерживает отдельный slot `critical_components_certificate` для сертификата соответствия критических компонентов.
...
- 2026-03-13 [application-form-source-ui-fixes] В `frontend/index.html`, `services/runtime/app/services/applicant_lookup_service.py`, `services/runtime/tests/test_applicant_lookup_service.py`, `services/runtime/tests/test_applications_api.py`, `services/runtime/tests/test_application_state_engine.py` и `PROJECT_MAP.md` applicant wizard Ордер 3 скорректирован под текущий UX-контракт: шаги `Заявитель` и `Адрес заявителя` объединены, пустые обязательные поля в applicant edit-режиме всегда получают soft-error background до submit, sourced lookup-поля отображаются серыми/read-only, длинный sidebar label `Документы для оценки соответствия` выровнен влево, а backend lookup перестал автозаполнять `applicant_head_position` из ГБД ЮЛ и начал дублировать Kompra ФИО в `applicant_head_name_kz`.
...
- 2026-03-12 [order3-application-form-analytics-fields] В `frontend/index.html` applicant wizard Ордер 3 переведен с демо-полей на аналитический состав формы: добавлены расширенные реквизиты заявителя/руководителя, адресный блок, полный блок ОПС, данные изготовителя и продукции, структурированное приложение, расширенный пакет документов и юридическое подтверждение; select-ы и ОПС lookup теперь загружаются из `reference-data` API, а автозаполнение по ОПС/аттестату синхронизируется в форме без изменения backend schema.
- 2026-03-12 [order3-applicant-document-s3-upload] В `services/runtime/app/services/file_slot_service.py`, `services/runtime/tests/test_files_api.py` и `frontend/index.html` applicant document package Ордер 3 переведен с сохранения имен файлов на реальный MinIO-backed upload flow: добавлены applicant application slots в `files-service`, валидация target entity для slot-ов, frontend upload выбранных файлов через `/files/slots/upload`, сохранение metadata в `payload.file_slots` и отображение статуса загруженных вложений в форме заявки.
- 2026-03-12 [order3-pdf-detailed-form-alignment] В `frontend/index.html` wizard Ордер 3 доведен до буквального соответствия `PDF_ORDERS_DETAILED.md`: обновлены точные названия label-ов, добавлены отсутствовавшие PDF-поля (`Вид сертификации` для объекта сертификации, длинные product identification поля, `Примечание (на казахском)`), applicant/manufacturer address selectors сделаны conditional по `Юридический/Другой`, приложение переведено в табличный вид без JSON-helper; в `services/runtime/app/seed/reference_data_seed.py` переименованы lookup items под PDF-термины и добавлены недостающие значения, а в `files-service` добавлен slot `critical_components_certificate`.
